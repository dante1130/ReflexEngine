 ...
 * @param transform Transformation from collision shape local-space to body
 * local-space
 * @param mass Mass of the collision shape (in kilograms)
 */
Collider::Collider(Entity entity, CollisionBody* body,
...
bool Collider::testPointInside(const Vector3& worldPoint) {
	const Transform localToWorld =
	    mBody->mWorld.mTransformComponents.getTransform(mBody->getEntity()) *
	...
	const CollisionShape* collisionShape =
	    mBody->mWorld.mCollidersComponents.getCollisionShape(mEntity);
...
void Collider::setCollisionCategoryBits(unsigned short collisionCategoryBits) {
	mBody->mWorld.mCollidersComponents.setCollisionCategoryBits(
	    mEntity, collisionCategoryBits);

	int broadPhaseId =
	    mBody->mWorld.mCollidersComponents.getBroadPhaseId(mEntity);
	...
	RP3D_LOG(mBody->mWorld.mConfig.worldName, Logger::Level::Information,
	         Logger::Category::Collider,
	         "Collider " + std::to_string(broadPhaseId) +
	             ": Set collisionCategoryBits=" +
...
/**
 * @param collideWithMaskBits The bits mask that specifies with which collision
 * category this shape will collide
 */
void Collider::setCollideWithMaskBits(unsigned short collideWithMaskBits) {
	mBody->mWorld.mCollidersComponents.setCollideWithMaskBits(
	    mEntity, collideWithMaskBits);
	...
	RP3D_LOG(
	    mBody->mWorld.mConfig.worldName, Logger::Level::Information,
	    Logger::Category::Collider,
	    "Collider" + std::to_string(broadPhaseId) +
	        ": Set collideWithMaskBits=" + std::to_string(collideWithMaskBits),
	    __FILE__, __LINE__);
}

// Set the local to  body transform
/**
 * @param transform The transform from local-space of the collider into the
 * local-space of the body
 */
void Collider::setLocalToBodyTransform(const Transform& transform) {
	mBody->mWorld.mCollidersComponents.setLocalToBodyTransform(mEntity,
	                                                           transform);

	// Update the local-to-world transform
	const Transform& bodyTransform =
	    mBody->mWorld.mTransformComponents.getTransform(mBody->getEntity());
	mBody->mWorld.mCollidersComponents.setLocalToWorldTransform(
	    mEntity, bodyTransform * transform);
	...
	RP3D_LOG(mBody->mWorld.mConfig.worldName, Logger::Level::Information,
	         Logger::Category::Collider,
	         "Collider " + std::to_string(getBroadPhaseId()) +
	             ": Set localToBodyTransform=" + transform.to_string(),
	         __FILE__, __LINE__);
}
...
bool Collider::raycast(const Ray& ray, RaycastInfo& raycastInfo) {
	...
	const Transform localToWorldTransform =
	    mBody->mWorld.mCollidersComponents.getLocalToWorldTransform(mEntity);
	const Transform worldToLocalTransform = localToWorldTransform.getInverse();
	Ray rayLocal(worldToLocalTransform * ray.point1,
	             worldToLocalTransform * ray.point2, ray.maxFraction);

	const CollisionShape* collisionShape =
	    mBody->mWorld.mCollidersComponents.getCollisionShape(mEntity);
	bool isHit = collisionShape->raycast(rayLocal, raycastInfo, this,
	...
	raycastInfo.worldNormal =
	    localToWorldTransform.getOrientation() * raycastInfo.worldNormal;
...
/**
 * @return The bits mask that specifies with which collision category this shape
 * will collide
 */
...
// Notify the collider that the size of the collision shape has been changed by
// the user
void Collider::setHasCollisionShapeChangedSize(
    bool hasCollisionShapeChangedSize) {
	mBody->mWorld.mCollidersComponents.setHasCollisionShapeChangedSize(
	    mEntity, hasCollisionShapeChangedSize);
}
...
void Collider::setMaterial(const Material& material) {
	...
	RP3D_LOG(mBody->mWorld.mConfig.worldName, Logger::Level::Information,
	         Logger::Category::Collider,
	         "Collider " + std::to_string(mEntity.id) + ": Set Material" +
	             material.to_string(),
	         __FILE__, __LINE__);
}
...
/**
 * @param isTrigger True if you want to set this collider as a trigger and false
 * otherwise
 */
