...
PointLight::PointLight(GLuint shadowWidth, GLuint shadowHeight,
	...
	: Light(shadowWidth, shadowHeight, color, aIntensity, dIntensity),
	  m_farPlane(far), m_position(position), 
...
std::vector<glm::mat4> PointLight::CalculateLightTransform() const
{
	...
	lightMatrices.push_back(m_lightProj * glm::lookAt(m_position, m_position + glm::vec3(1.0, 0.0, 0.0), glm::vec3(0.0, -1.0, 0.0)));
	lightMatrices.push_back(m_lightProj * glm::lookAt(m_position, m_position + glm::vec3(-1.0, 0.0, 0.0), glm::vec3(0.0, -1.0, 0.0)));

	lightMatrices.push_back(m_lightProj * glm::lookAt(m_position, m_position + glm::vec3(0.0, 1.0, 0.0), glm::vec3(0.0, 0.0, 1.0)));
	lightMatrices.push_back(m_lightProj * glm::lookAt(m_position, m_position + glm::vec3(0.0, -1.0, 0.0), glm::vec3(0.0, 0.0, -1.0)));

	lightMatrices.push_back(m_lightProj * glm::lookAt(m_position, m_position + glm::vec3(0.0, 0.0, 1.0), glm::vec3(0.0, -1.0, 0.0)));
	lightMatrices.push_back(m_lightProj * glm::lookAt(m_position, m_position + glm::vec3(0.0, 0.0, -1.0), glm::vec3(0.0, -1.0, 0.0)));
