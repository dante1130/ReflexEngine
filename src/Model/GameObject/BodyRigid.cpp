...
void BodyRigid::update(double delta_time) {}

void BodyRigid::fixed_update(double delta_time) {
...
void BodyRigid::addBoxCollider(glm::vec3 pos, glm::vec3 halfSize,
	...
	m_box.emplace_back(
	    colliderData_box(m_numOfColliders, halfSize.x, halfSize.y, halfSize.z));
	m_numOfColliders++;
}
...
void BodyRigid::addSphereCollider(glm::vec3 pos, float radius, float bounciness,
	...
	m_sphere.emplace_back(colliderData_sphere(m_numOfColliders, radius));
	m_numOfColliders++;
}
...
void BodyRigid::addCapsuleCollider(glm::vec3 pos, float radius, float height,
	...
	m_capsule.emplace_back(
	    colliderData_capsule(m_numOfColliders, radius, height));
	m_numOfColliders++;
}
