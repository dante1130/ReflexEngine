...
namespace gui
{
	////METHOD NEEDED FOR GUI TO OPERATE////
		...
		* @param	openglVersion	- String of opengl version. I.e., OpenGL 4.6 uses "#version 460", check imGUI documentation if unsure
		* 
		* @return	Void
		*/
		...
		/**
		* @brief	Does per frame initialise for gui. Should be placed after glClear & glClearColor in the main loop
		* @param	No param
		* @return	Void
		*/
		void mainLoopStart();
	...
	////GUI WINDOW OPERATION////
		/**
		* @brief	Begins a window. Sandwich inside gui window methods between begin and end.
		...
		*/
		...
		/**
		* @brief	Ends the window. Sandwich inside gui window methods between begin and end.
		* @param	No param
		* @return	Void
		*/
		void end();
	...
	////ITEMS INSIDE GUI WINDOW////
		...
		* @return	void
		*/
		void plotLines(const std::string name, const float* values, int numOfEntries, float xWindowSize, float yWindowSize);
		...
		* @return	void
		*/
		void plotLines(const char* name, const float* values, int numOfEntries, float xWindowSize, float yWindowSize);
		...
		* @return	void
		*/
		void plotLines(const std::string name, const float *values, int numOfEntries, float minVal, float maxVal, float xWindowSize , float yWindowSize);
		...
		* @return	void
		*/
		void plotLines(const char* name, const float* values, int numOfEntries, float minVal, float maxVal, float xWindowSize, float yWindowSize);
	...
	enum
	{
		...
		/// <summary>
		/// Window constraints enforced on program execution. I.e., window size & pos
		/// </summary>
		SET_WINDOW_CONSTRAINTS_ON_EXECUTION = 2,

		/// <summary>
		/// Window constraints enforced on first ever launch. I.e., window size & pos
		/// </summary>
