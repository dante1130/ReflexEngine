...
void gui::begin(const char* title) { ImGui::Begin(title); }
...
bool gui::button(const std::string name) { return button(name.c_str()); }
...
void gui::plotLines(const std::string name, const float* values,
                    ...
                    float xWindowSize, float yWindowSize) {
	plotLines(name.c_str(), values, numOfEntries, minVal, maxVal, xWindowSize,
	          yWindowSize);
}
...
void gui::plotLines(const char* name, const float* values, int numOfEntries,
                    ...
                    float yWindowSize) {
	ImGui::PlotLines(name, values, numOfEntries, 0, "", minVal, maxVal,
