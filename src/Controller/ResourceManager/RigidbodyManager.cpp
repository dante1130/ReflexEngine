...
void RigidbodyManager::add_rigidbody(Component::Rigidbody& rb, const Component::Transform& tf)
{
	rb.init(true, tf.position, tf.rotation);
	prev_position = tf.position;
}

void RigidbodyManager::update_rigidbody(Component::Rigidbody& rb, Component::Transform& tf)
{
	...
	if (rb.getCanSleep() != rb.can_sleep)
		rb.setCanSleep(rb.can_sleep);

	if (rb.getDragForce() != rb.lin_drag)
		rb.setDragForce(rb.lin_drag);
	if (rb.getDragTorque() != rb.ang_drag)
		rb.setDragTorque(rb.ang_drag);
